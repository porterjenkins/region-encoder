...
class AutoEncoder(nn.Module):
    ...
    def add_noise(image_tensor, noise_factor=.5):
        ...
        noise = torch.randn((batch_size, channels, h, w)).cuda()
    ...
    def run_train_job(self, n_epoch, img_tensor):
        ...
        for epoch in range(n_epoch):  # loop over the dataset multiple times
            for step in range(int(n_samples / batch_size)):
                ...
                noisey_inputs = AutoEncoder.add_noise(img, noise_factor=.25)
...
if __name__ == "__main__":
    ...
    img_tensor = torch.Tensor(region_grid.img_tensor).cuda()

    auto_encoder = AutoEncoder(img_dims=(640, 640), h_dim_size=32).cuda()
